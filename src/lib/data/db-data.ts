    ...
    const taskHeads = project.taskHeads.map<TaskHeadSummary>((head) => {
      const tasks = head.tasks.map<TaskSummary>((task) => {
        ...
        const taskRecord = {
          ...
          supervisor_workers: task.supervisorWorkers,
          foreman_workers: task.foremanWorkers,
          carpenter_workers: task.carpenterWorkers,
          mason_workers: task.masonWorkers,
          steelman_workers: task.steelmanWorkers,
          welder_workers: task.welderWorkers,
          painter_workers: task.painterWorkers,
          operator_workers: task.operatorWorkers,
          ...
          time_hours: toNumber(task.timeHours),
        ...
        const outputs = calculateProductivityOutputs({
          ...
          hoursPerDay: taskRecord.time_hours,
